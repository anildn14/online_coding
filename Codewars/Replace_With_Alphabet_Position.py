...
def alphabet_position(text):
    return ' '.join(str(int(ord(x)-96)) for x in text.lower() if x.isalpha()) 	#SELF
    # return ' '.join(str(ord(c) - 96) for c in text.lower() if c.isalpha())	#CLEVER
    # return " ".join(str(ord(c)-ord("a")+1) for c in s.lower() if c.isalpha())	#CLEVER
